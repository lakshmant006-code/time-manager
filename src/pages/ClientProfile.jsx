    ...
    <AppShell>
      <div style={{ background: '#fff', borderRadius: 8, padding: 32, boxShadow: 'var(--shadow-card)', fontFamily: 'var(--font-sans)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4, flexWrap: 'wrap', gap: 12 }}>
          <div>
            <h2 style={{ fontSize: 24, fontWeight: 600, fontFamily: 'var(--font-display)', margin: 0 }}>{client.Client_name}</h2>
            <p style={{ color: 'var(--text-secondary)', marginTop: 4 }}>Client ID: {client.Client_ID}</p>
          </div>
          <Badge tone={client.Client_Status ? 'success' : 'neutral'}>{client.Client_Status ? 'Active' : 'Inactive'}</Badge>
        </div>
